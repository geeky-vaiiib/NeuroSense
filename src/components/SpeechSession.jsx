...
/* ──────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────── */
export default function SpeechSession({ onComplete, onSkip }) {
  ...
  /* ── Finish session params ref ─────────────────────────── */
  const finishParamsRef = useRef({ audioBlob, cleanup, onComplete, transcriptHint });
  useEffect(() => {
    finishParamsRef.current = { audioBlob, cleanup, onComplete, transcriptHint };
  ...
  const finishSession = useCallback(() => {
    ...
    setPhase('processing');
    setTimeout(() => {
      const { audioBlob: b, cleanup: cl, onComplete: oc, transcriptHint: th } = finishParamsRef.current;
      cl();
      const blob = b || new Blob(chunksRef.current, { type: preferredMime() || 'audio/webm' });
      oc(blob, th);
    }, 2000);
  }, []);
